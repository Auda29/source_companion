...
- Implementierungsnotiz: CI-Run `27226750569` vom 2026-06-09 ist weiter fehlgeschlagen. Job `Tauri Rust check` scheitert bei `cargo fmt --check`, weil `src-tauri/src/lib.rs` in `fn invoke` noch rustfmt-Zeilenumbruch braucht: `self.worker.invoke(...)` muss auf die mehrzeilige rustfmt-Form gebracht werden. Job `Node tests and desktop assets` scheitert bei `npm run ci:node` mit 112/114 Tests; betroffen sind `tests/desktop-bridge.test.js:198` / Assertion bei Zeile 207 fuer Clone ueber Operation Queue und `tests/desktop-bridge.test.js:332` / Assertion bei Zeile 375 fuer Publish ueber backend-only GitHub Client und Operation Queue. Beide Assertions erwarten `true`, erhalten aber `false`. Erst lokal reproduzieren, dann gezielt `src-tauri/src/lib.rs`, `src/desktop-bridge.js` und die betroffenen Tests pruefen. Keine CI-Abschwaechung vornehmen, ausser ein Check ist nachweislich falsch modelliert.
- Notiz: 2026-06-10 07:43 CEST umgesetzt: `src-tauri/src/lib.rs` bringt `DesktopBridgeState::invoke` in die mehrzeilige rustfmt-Form. Clone-/Publish-Pfadvalidierung nutzt jetzt eine hostunabhaengige Absolute-Path-Pruefung fuer POSIX-, Windows-Drive- und UNC-Pfade, damit die Desktop-Bridge-Queue-Tests auf Linux-CI nicht vor der Queue-Ausfuehrung an `path.isAbsolute` scheitern. Neue Tests decken Windows-absolute Clone-Targets und Publish-Folder auf non-Windows-CI ab. Fokussierte Desktop-Bridge-/Clone-/Publish-Tests, `npm run ci:node` mit 116/116 Tests und `git diff --check` bestanden lokal. `cargo fmt --check` konnte lokal nicht ausgefuehrt werden, weil `cargo`/`rustfmt` in dieser Umgebung nicht installiert sind.
- Review-Ergebnis: Zurueckgestellt am 2026-06-10 08:12 CEST: Lokale Code-Review der Pfadvalidierungs-/rustfmt-Aenderung ohne neuen Blocker; `npm run ci:node` bestand mit 116/116 Tests und `git diff --check` bestand. `cargo`/`rustfmt` ist lokal nicht installiert. GitHub-CLI ist durch Netzwerk-Sandbox blockiert, und der GitHub-Connector lieferte fuer `HEAD` `98003ef7cf74ab76ec4dc83683f13fd4abf9313c` keine Workflow-Run-/Statusdaten.
- Offene Review-Punkte: Aktuelle Aenderungen pushen oder CI neu starten; danach nachweisen, dass der naechste Push-/PR-CI-Run gruen ist, insbesondere `Tauri Rust check` mit `cargo fmt --check`.
