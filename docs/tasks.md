...
- Implementierungsnotiz: CI-Run `27257201853` vom 2026-06-10 ist weiter fehlgeschlagen, aber die vorherigen Node- und Rustfmt-Blocker sind behoben: Job `Node tests and desktop assets` ist gruen, und im Job `Tauri Rust check` besteht `cargo fmt --check`. Der verbleibende Fehler liegt im Schritt `Check Tauri crate`: `cargo check` scheitert bei `tauri::generate_context!()` in `src-tauri/src/lib.rs:614`, weil `frontendDist` auf `"../desktop-dist"` zeigt, dieser Ordner im Rust-Job aber nicht existiert. Vermutlich muss der Tauri-Rust-Job vor `cargo check` Node-Abhaengigkeiten installieren oder mindestens `npm run desktop:assets` ausfuehren, damit `desktop-dist` erzeugt wird. Keine CI-Abschwaechung vornehmen; der Rust-Check soll die echte Tauri-Konfiguration mit vorhandenen Desktop-Assets pruefen.
- Notiz: 2026-06-10 08:21 CEST umgesetzt: `.github/workflows/ci.yml` richtet im Tauri-Rust-Job Node 22 ein und fuehrt vor `cargo check` `npm run desktop:assets` aus, damit der von `tauri.conf.json` referenzierte Ordner `desktop-dist` im Rust-Job existiert. `npm run desktop:assets`, `npm run ci:node` und `git diff --check` bestanden lokal; `cargo fmt --check`/`cargo check` konnten lokal nicht ausgefuehrt werden, weil `cargo`/`rustfmt` in dieser Umgebung nicht installiert sind. Der naechste GitHub-Actions-Run muss `Tauri Rust check` remote bestaetigen.
- Review-Ergebnis: Bestanden am 2026-06-10. Der Tauri-Rust-Job erzeugt vor `cargo check` per `npm run desktop:assets` den von `src-tauri/tauri.conf.json` referenzierten Ordner `desktop-dist`; der Asset-Build nutzt nur Node-Built-ins und benoetigt deshalb im Rust-Job kein `npm install`. `npm run ci:node` bestand lokal mit 116/116 Tests, und `git diff --check` meldete keine Whitespace-Fehler. `cargo fmt --check`/`cargo check` konnten lokal nicht ausgefuehrt werden, weil `cargo`/`rustfmt` in dieser Umgebung nicht installiert sind; die GitHub-Connector-Abfrage fand fuer den uncommitted lokalen Patch keinen PR-Workflow-Run.
- Offene Review-Punkte: -
